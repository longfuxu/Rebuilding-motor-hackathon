...
## §10 — Whole-protein circular-permutation closure screen (systematic, not a lucky hit)

Screened **20 CP cut sites across the whole gp16 protein** (14 whole-protein + 6 carried from the 220–240 grid) (cut_penalty 6–198, all func_clear), spanning the
N-domain interior, the hinge, and the C-domain, to test whether ring closure is a systematic region-defined
property or a chance single hit at cp233.

**Stage 1 (monomer gate):** all 20 CP subunits re-fold to native gp16 (N 2.4–3.6 Å, C 2.2–7.1 Å) — even the
deepest N-domain interior cuts (cp60 pen198, cp75 pen196). *The monomer fold does not discriminate by cut-penalty;
the discriminating test is ring closure.*

**Stage 2 (pentamer, sequential M2 + M1, PASS = both predictors ≥4/5):**

| region | sites closing / total | closing sites |
|---|---|---|
| N-domain (4–200) | 1/7 | cp183 |
| hinge (201–228) | 0/3 | — |
| C-domain (229–330) | 7/10 | 230,232,233,234,236,285,297 |

**The rule is region-gated, not penalty-gated.** Closure vs raw cut_penalty is weak (point-biserial r=−0.34,
p=0.20). The two *lowest*-penalty hinge sites (cp228 pen6, cp217 pen33) do NOT robustly close, while mid-penalty
C-domain sites (cp285 pen61, cp297 pen60) do — the productive variable is which folding unit the cut lands in.

**Robustness vs cp233:** cp234, cp236, cp285 match cp233 (close at all 3 inter-linkers 10/15/20); none clearly
beats it. cp233 sits in a contiguous C-domain closure window (230–236) **plus** an independent second C-domain
site (cp285) — a systematic, reproducible region-defined result.

**One-line conclusion:** *A whole-protein screen of 20 circular-permutation cut sites (penalty 6–198) shows ring
closure is gated by folding-unit region — the C-domain is productive (7/10 close, including a new robust site
cp285), the N-domain and hinge are not — proving cp233 is a systematic, region-defined hit rather than a chance
success.* Best design remains **cp233_int15_inter10**. (Free NIM; sequential M2 not global pTM; apo; cross-checked.)

Artifacts: fig11_cp_screen_landscape.png, cp_screen_landscape.csv, cp_screen_site_summary.csv,
cp_screen_wholeprotein_stage1.csv, cp_screen_finding.txt.


## §11 — Genetically addressable dead-seat on the single-chain lead (core deliverable)

Chassis: **cp233_int15_inter10** (1750 aa, definitive single-chain lead). Walker-B **E119Q** (catalytic
Glu→Gln) placed at defined seats; sequential M2 + M1 closure, both predictors, apo, free NIM.

| construct | Boltz | OF3 | M2 | M1 |
|---|---|---|---|---|
| WT cp233 | 3/3 | 5/5 | PASS | YES |
| E119Q {3} (single, ring center) | 2/3 | 5/5 | PASS | YES |
| E119Q {1} | 3/3 | 5/5 | PASS | YES |
| E119Q {1,3} | 2/3 | 5/5 | PASS | YES |
| E119Q {1,3,5} | 3/3 | 5/5 | PASS | YES |
| E119Q {all 5} | 2/3 | 5/5 | PASS | YES |
| R146A {3} (position control) | 3/3 | 4/5* | PASS | YES |

**1) E119Q tolerated at every defined seat** — single, multi, up to all 5 — with the ring remaining closed
under both predictors. A single gene encodes a ring whose catalytic seats are inactivated at chosen positions
(the project's core single-chain addressable-inactivation deliverable, analogous to single-chain ClpX).

**2) No progressive geometric collapse:** median trans-R146 stays 5.0–6.3 Å independent of dead-seat count
(0→5); M1 ring-closure = 100% throughout. E119Q abolishes catalysis, not the geometric seat (matches
B1_L40_E119Q). The progressive-opening model concerns ATP-loading dynamics, not a static effect of removing
the catalytic glutamate.

**3) Position control (R146A @ seat 3):** removing the trans arginine finger makes interface 3→4 unmeasurable
(no guanidinium atoms) while the other four interfaces stay engaged (~5.6 Å) — **M2 localizes to exactly the
edited interface**, direct evidence of positional control.

**Conclusion:** On the definitive single-chain lead, Walker-B E119Q is a genetically addressable, position-
controlled dead seat — tolerated singly or in any pattern up to all 5, ring closed under both predictors, with
the R146A control proving the readout resolves the edited interface. Zero generative design.
**Next:** WT / E119Q{3} / E119Q{all 5} → AlphaFold3 third predictor.

Artifacts: fig12_deadseat_addressable.png, deadseat_summary.csv, deadseat_trend.csv, deadseat_finding.txt.


*R146A {3} OF3 engages 4/5 interfaces — the 5th (3→4) is unmeasurable by design (the arginine finger was deleted, so no guanidinium atom exists to score); this is the localization signal, not a closure failure. M2 PASS (4/5 ≥ 4).
